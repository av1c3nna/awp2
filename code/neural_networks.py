...
class Trainer(model_utils.BaseModel):
    def __init__(self, feature_engineerer, model_class, quantiles, optimizer_class=torch.optim.Adam, in_shape=50, dropout=0.1, weight_decay=1e-6, lr = 0.01, batch_size = 96, load_pretrained=False, model_path="model.pth"):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.quantiles = quantiles
