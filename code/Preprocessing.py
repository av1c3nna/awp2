...
class FeatureEngineerer:
    ...
    def perform_feature_engineering(self, data, deployment:bool = False, labels_to_remove:list = ["Solar_MWh_credit", "Wind_MWh_credit"], val_year=False):
        ...
        if deployment:
            ...
            self.train_val_test_split_hardcoded(data, val_year)
    ...
    def train_val_test_split_hardcoded(self, data, val_year):
        ...
        if val_year:
            train_data = data[data.index.date < date(2022, 7, 1)]
            val_data = data[(data.index.date >= date(2022, 7, 1)) & (data.index.date < date(2023, 7, 1))]
            test_data = data[(data.index.date >= date(2023, 7, 1)) & (data.index.date < date(2024, 1, 1))]
